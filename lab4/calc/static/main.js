...
document.querySelector(".buttons").onclick = (event) => {
  ...
  if (key == "AC") {
    ...
    if (
      String(calculated).includes(".") &&
      String(calculated).split(".")[1].length > 9
    ) {
      out.textContent = calculated.toFixed(6);
      logger = [calculated.toFixed(6)];
    } else {
      out.textContent = calculated;
      logger = [calculated];
    }
  } else if (key in action) {
    if (squenceDigits != "") logger.push(squenceDigits);
    squenceDigits = String();
